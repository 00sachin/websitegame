...
def gameproduct(request,myid ):
    if request.method == "POST":
        ...
        order = Orders(name=name, email=email, message=message, phone=phone ,games=games ,price=price)
        order.save()
        id = order.id
        subject = "congralation for buying the game"  
        html_message = render_to_string('bill.html',{'name': name, 'id':id, 'phone': phone,'email': email,'games': games,'price': price})
